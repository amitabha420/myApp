...
exports.EnlistContentAccess = function(req,res)
{
	...
	StatContentAccess.channelid = input.channelid;
	StatContentAccess.contentImageUrl = input.contentImageUrl;
...
//number of new Content Weekly, Monthly and Daily.
/*
INPUT
{
    "fromDate" : ["2015","06","03"],
    "contentAccessType" : "Streaming",
    "contentId" : "552fc6fb8a5ec0b0060bf3b0",
    "countType" : "Daily"
}
*/
exports.getContentAccessCount_Weekly_Monthly_Daily = function(req,res)
{
    //res.send("Ok");
    var input = req.body;
    var _fromDate = input.fromDate;
    var contentAccessType=input.contentAccessType;
    var contentId=input.contentId;
    var countType=input.countType;

    //StatContentAccessSchema
    if(contentAccessType.toLowerCase() === "Streaming".toLowerCase())
    {
        if(countType.toLowerCase() === "Daily".toLowerCase())
        {
          var fromDate = new Date(parseInt(_fromDate[0]),parseInt(_fromDate[1])-1,parseInt(_fromDate[2]));
          var endDate = new Date(parseInt(_fromDate[0]),parseInt(_fromDate[1])-1,parseInt(_fromDate[2])+1);
          ...
          StatContentAccessSchema.find({"CreateDate": {"$gte": fromDate, "$lt": endDate},"contentid":contentId,"IsDownloaded":false}).count().exec(
                    function(err,result)
                 {
                    if(err)
                        res.send(result);      
        ...
        else if(countType.toLowerCase()  === "Weekly".toLowerCase())
        {
            var curr = new Date(parseInt(_fromDate[0]),parseInt(_fromDate[1])-1,parseInt(_fromDate[2]));
            var currDay=curr.getDay();
            var firstday = new Date(curr.getTime() - ((parseInt(currDay)-1) * 24 * 60 * 60 * 1000));
            ...
            StatContentAccessSchema.find({"CreateDate": {"$gte": firstday, "$lte": lastday},"contentid":contentId,"IsDownloaded":false}).count().exec(
            function(err,result)
             {
                if(err)
                    res.send(result);      
        ...
        else
        {
            ...
            StatContentAccessSchema.find({"CreateDate": {"$gte": startDate, "$lte": endDate},"contentid":contentId,"IsDownloaded":false}).count().exec(
            function(err,result)
             {
                if(err)
                    res.send(result);      
    ...
    if(countType.toLowerCase() === "Daily".toLowerCase() )
        {
            ...
            var endDate = new Date(parseInt(_fromDate[0]),parseInt(_fromDate[1])-1,parseInt(_fromDate[2])+1);
              ...
              StatContentAccessSchema.find({"CreateDate": {"$gte": fromDate, "$lt": endDate},"contentid":contentId,"IsDownloaded":true}).count().exec(
                        function(err,result)
                     {
                        if(err)
                            res.send(result);      
            ...
            else if(countType.toLowerCase() === "Weekly".toLowerCase())
            {
                ...
                StatContentAccessSchema.find({"CreateDate": {"$gte": firstday, "$lte": lastday},"contentid":contentId,"IsDownloaded":true}).count().exec(
                function(err,result)
                 {
                    if(err)
                        res.send(result);      
            ...
            else
            {
                ...
                StatContentAccessSchema.find({"CreateDate": {"$gte": startDate, "$lte": endDate},"contentid":contentId,"IsDownloaded":true}).count().exec(
                function(err,result)
                 {
                    if(err)
                        res.send(result);      
